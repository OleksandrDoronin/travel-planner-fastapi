...
from src.auth.services.google_oauth_url_generator import GoogleOAuthUrlGenerator
...
async def google_login(
    request: Request,
    google_oauth_url_generator: Annotated[GoogleOAuthUrlGenerator, Depends(GoogleOAuthUrlGenerator)],
    redirect_uri: HttpUrl = Query(..., alias='redirect_uri'),
) -> GoogleLoginResponse:
    try:
        state = generate_random_state()
        request.session['state'] = state
        google_auth_url = google_oauth_url_generator.get_google_auth_url(redirect_uri=redirect_uri, state=state)
